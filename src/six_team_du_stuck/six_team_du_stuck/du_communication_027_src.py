...
# ros2 run multi_du_udpcomm_02_pkg du_communication_027 --ros-args --params-file src/multi_du_udpcomm_02_pkg/config/params_027.yaml
...
UDP_IP_VORTEX = config['IP']['ip_vortex_027']
UDP_IP_ROS = config['IP']['ip_du_027']
UDP_SEND_PORT = int(config['IP']['port_vortex_027'])
UDP_RECEIVE_PORT = int(config['IP']['port_du_027'])
...
class MinimalPubSubComm(Node):
    ...
    def __init__(self):
        # Initialization
        super().__init__('du_communication_027')       
        
        self.declare_parameter('param_turtlePublish_communication', 'du_027/pose')
        self.declare_parameter('param_turtleSubsc_communication', 'du_027/cmd_vel')
        ...
        self.status_vessel_angle_pub = self.create_publisher(Float32, 'du_027/vessel_angle', 100)
        ...
        # Subscribers
        #self.subscription = self.create_subscription(Twist, 'du_027/cmd_vel', self.subscription_callback, 10)
        self.subscription = self.create_subscription(Twist, turtleSubsc, self.subscription_callback, 10)
        self.vessel_angular_velocity_sub = self.create_subscription(Float32, 'du_027/vessel_angular_velocity', self.vessel_angular_velocity_callback, 10)
        self.subscription_joint = self.create_subscription(JointState, 'du_027/cmd_joint', self.subscription_joint_callback, 10)
