...
# ros2 run two_team_du_test du_controller_016 --ros-args --params-file src/two_team_du_test/config/params_016.yaml
...
class MinimalPubSubCont(Node):
    ...
    def __init__(self):   
        # Initialization
        super().__init__('du_controller_016') #node name
                      
        # Publishers
        self.twist_pub = self.create_publisher(Twist, 'du_016/cmd_vel', 100)
        self.vessel_angular_velocity_pub = self.create_publisher(Float32, 'du_016/vessel_angular_velocity', 10)
        self.cmd_joint_pub = self.create_publisher(JointState, 'du_016/cmd_joint', 100)
        # Excavator Publishers 
        self.flag_work_start_pub = self.create_publisher(Bool,'ex_003/flag/work/start',100)
        self.flag_work_arrival_pub = self.create_publisher(Int16,'ex_003/flag/work/arrival',100)
        self.flag_dump_site_arrival_pub = self.create_publisher(Int16,'ex_003/flag/dump_site/arrival',100)
        self.flag_dump_site_finished_pub = self.create_publisher(Bool,'ex_003/flag/dump_site/finished',100)
        ...
        # Subscribers
        self.subscription = self.create_subscription(Pose, 'du_016/pose',  self.subsc_callback, 10)
        self.subscription_012 = self.create_subscription(Pose, 'du_012/pose',  self.subsc_callback_012, 10)
        self.vessel_angle_sub = self.create_subscription(Float32,'du_016/vessel_angle',self.vessel_angle_callback,10)
        # Excavator Subscribers
        self.flag_startup_sub = self.create_subscription(Bool,'du_016/flag/startup', self.flag_work_start_callback, 10)
        self.flag_work_finish_sub = self.create_subscription(Bool,'ex_003/flag/work/finish', self.flag_work_finish_callback, 10)
        self.flag_work_entry_sub = self.create_subscription(Bool,'ex_003/flag/work/entry',self.flag_work_entry_callback, 10)
        self.flag_stack_sub = self.create_subscription(JointState,'du_016/flag/stack',self.flag_stack_callback, 10)
        self.flag_dump_site_entry_sub = self.create_subscription(Bool,'ex_003/flag/dump_site/entry',self.flag_dump_site_entry_callback,10)
    ...
    def timer_callback(self):
        ...
        goal = [(-20.0, 35.0),(-10.0, 55.0),(-5.0,35.0),(-135.0,55.0),(-145.0,35.0),(-150.0,55.0)]
        ...
        if self.state == 0:
            ...
            if self.current_vessel_angle_rad < -1.60:
                vessel_angular_velocity_tmp.data = 0.0
                self.throw_away_count += 1
                if self.throw_away_count > 400:
        ...
        # speed control
        if math.hypot(self.turtlePos[0] - self.obstacle_x,self.turtlePos[1] - self.obstacle_y) < 15:
