...
config.read('src/four_team_ex_cnp/config/config.ini')

UDP_IP_VORTEX = config['IP']['ip_vortex_004']
UDP_IP_ROS = config['IP']['ip_ex_004']
UDP_SEND_PORT = int(config['IP']['port_vortex_004'])
UDP_RECEIVE_PORT = int(config['IP']['port_ex_004'])
...
class MinimalPubSubComm(Node):
    ...
    def __init__(self):
        # Initialization
        super().__init__(f'ex_communication_{self.ex_team_number}')
        
        # Publishers
        self.status_joint_pub = self.create_publisher(JointState, f'ex_{self.ex_team_number}/status/joint/ang', 100)
        self.ex_soilmass_pub = self.create_publisher(Float32, f'ex_{self.ex_team_number}/soilmass', 100)
        self.ex_pose_pub = self.create_publisher(Pose, f'ex_{self.ex_team_number}/pose', 100)
        ...
        # Subscribers        
        self.subscription = self.create_subscription(Twist, f'ex_{self.ex_team_number}/cmd/body/vel', self.subscription_callback, 10)
        self.subscription_ang = self.create_subscription(Twist, f'ex_{self.ex_team_number}/cmd/ang/vel', self.subscription_ang_callback, 10)
        self.cmd_swing_ang_sub = self.create_subscription(Float32, f'ex_{self.ex_team_number}/cmd/swing/ang', self.cmd_swing_ang_callback, 10)
        self.cmd_boom_ang_sub = self.create_subscription(Float32, f'ex_{self.ex_team_number}/cmd/boom/ang', self.cmd_boom_ang_callback, 10)
        self.cmd_arm_ang_sub = self.create_subscription(Float32, f'ex_{self.ex_team_number}/cmd/arm/ang', self.cmd_arm_ang_callback, 10)
        self.cmd_bucket_ang_sub = self.create_subscription(Float32, f'ex_{self.ex_team_number}/cmd/bucket/ang', self.cmd_bucket_ang_callback, 10)
