...
from four_team_ex_cnp.a_star import astar
from four_team_ex_cnp.pure_pursuit import pure_pursuit_method
import numpy as np
...
class MinimalPubSubCont(Node):
    ...
    ex_team_number = 2
    soil_hill_position = [[40.0, -25.0]]
    dump_site_position = [-75.0,-55.0]

    loading_time = 40.0
    mounding_time = 10.0
    distance = (abs(soil_hill_position[0][0]-dump_site_position[0])+abs(soil_hill_position[0][1]-dump_site_position[1]))*4.0
    dumpspeed = 10.0
    ave_sand = 750.0
    units = 0
    add_units = 0
    ...
    request_to_server = Int16()
    reqeust_all_dump_team_list_flag = Int16()
    ex_pos_team_number = ExPosAndTeamNumber()
    ex_pos_team_number.dumpsite = dump_site_position
    ...
    def __init__(self):   
        # Initialization
        super().__init__(f'ex_controller_{self.ex_team_number}') #node name
        # Publishers
        self.cmd_vel_pub = self.create_publisher(Twist, f'ex_{self.ex_team_number}/cmd/body/vel', 100)
        self.cmd_ang_pub = self.create_publisher(Twist, f'ex_{self.ex_team_number}/cmd/ang/vel', 100)
        self.cmd_swing_ang_pub = self.create_publisher(Float32, f'ex_{self.ex_team_number}/cmd/swing/ang', 100)
        self.cmd_boom_ang_pub = self.create_publisher(Float32, f'ex_{self.ex_team_number}/cmd/boom/ang', 100)
        self.cmd_arm_ang_pub = self.create_publisher(Float32, f'ex_{self.ex_team_number}/cmd/arm/ang', 100)
        self.cmd_bucket_ang_pub = self.create_publisher(Float32, f'ex_{self.ex_team_number}/cmd/bucket/ang', 100)
        self.twist_pub = self.create_publisher(Twist, f'ex_{self.ex_team_number}/cmd_vel', 100)
        # Dump Publishers
        self.flag_work_entry_pub = self.create_publisher(Bool,f'ex_{self.ex_team_number}/flag/work/entry',100)
        self.flag_dump_site_entry_pub = self.create_publisher(Bool,f'ex_{self.ex_team_number}/flag/dump_site/entry',100)
        self.flag_work_finish_pub = self.create_publisher(Bool,f'ex_{self.ex_team_number}/flag/work/finish',100)
        self.flag_task_complete_pub = self.create_publisher(Bool,f'ex_{self.ex_team_number}/flag/task_complete',100)
        # Server Publishers
        self.request_from_ex = self.create_publisher(Int16,'request_from_ex',100)
        self.reqeust_all_dump_team_list = self.create_publisher(Int16, 'reqeust_all_dump_team_list',10)
               
        # Joy Subscribers
        self.status_joint_sub = self.create_subscription(JointState, f'ex_{self.ex_team_number}/status/joint/ang',  self.status_joint_callback, 10)
        self.cmd_joy_sub = self.create_subscription(Joy, f'ex_000/cmd/joy',  self.cmd_joy_callback, 10)
        self.ex_soilmass_sub = self.create_subscription(Float32, f'ex_{self.ex_team_number}/soilmass', self.ex_soilmass_callback, 100)
        self.subscription_ex = self.create_subscription(Pose, f'ex_{self.ex_team_number}/pose',  self.subsc_callback_ex, 10)
        # Dump Subscrivers
        self.flag_work_arrival_sub = self.create_subscription(Int16,f'ex_{self.ex_team_number}/flag/work/arrival',self.flag_work_arrival_callback,10)
        self.flag_work_start_sub = self.create_subscription(Bool,f'ex_{self.ex_team_number}/flag/work/start',self.flag_work_callback,10)
        self.flag_dump_site_arrival_sub = self.create_subscription(Int16,f'ex_{self.ex_team_number}/flag/dump_site/arrival',self.flag_dump_site_arrival_callback,10)
        self.flag_dump_site_finished_sub = self.create_subscription(Bool,f'ex_{self.ex_team_number}/flag/dump_site/finished',self.flag_dump_site_finished_callback,100)
        self.flag_stack_sub = self.create_subscription(JointState,f'ex_{self.ex_team_number}/flag/stack',self.flag_stack_callback,10)
        self.bit_from_dump = self.create_subscription(DumpCNP,f'ex_{self.ex_team_number}/bit_on_ex',self.bit_from_dump,10)
        # Server Subscrivers
        self.response_from_server = self.create_subscription(Int64MultiArray, f'ex_{self.ex_team_number}/response_from_server',self.response_from_server_callback,10)
        self.all_dump_team_list_from_server = self.create_subscription(Int64MultiArray, f'ex_{self.ex_team_number}/all_dump_team_list_from_server', self.all_dump_team_list_from_server_callback ,10)
    ...
    def bit_from_dump(self, DumpCNP):
        ...
        if len(self.dump_CNP_list) == len(self.dump_ID_list):
            ...
            for i in range(self.add_units):
                ...
                self.get_logger().info(f'du_{du_id}/decide_team')
                team_number = Int16()
                team_number.data = self.ex_team_number
                decide_team_of_dump.publish(team_number)
                print('Request to', du_id)
                time.sleep(0.1)
            print("finished sorting and requested")
    ...
    def response_from_server_callback(self,idle_list):
        ...
        if self.task_flag == True:
            if len(self.initial_dump_list) == 0:
                ...
                self.get_logger().info(f"{self.initial_dump_list}")
            for i in self.dump_ID_list:
                request_dump_topic = f'du_{i}/request_from_ex'
                flag_request_pub = self.create_publisher(Int16,request_dump_topic,10)
                flag_request_tmp = Int16()
                flag_request_tmp.data = self.ex_team_number
    ...
    def cmd_joy_callback(self, joy_tmp):
        ...
        # adjust angle
            ...
            if self.turtlePos[2] < 30:
                ...
                # request to server
                self.task_flag = True
                self.request_to_server.data = self.ex_team_number
        ...
        # dig up
            ...
            if self.arrival_flag:   
                ...
                if ko_state == 10:
                    state = state + 1
                    ko_state = 0
                    # don't dig in the same spot
                    if state == 1:
                        self.swing_diff_target += 5.0
                    # calculate remaining soil mass
                    elif state == 7:
                        self.sand -= soilmass
                        ...
                        time.sleep(0.1)
                        self.flag_work_finish_pub.publish(flag_work_finish_tmp) # publish two times
                        # Initialize
            ...
            if self.request_flag:
                ...
                if time.time() - self.interval_time > 20.0: # original 22.0
                    ...
                    try:
                        ...
                        dump_topic = 'du_{}/flag/startup'.format(self.sorted_dump_list[self.request_units].id) # attention!!!
                        print(dump_topic)
        ...
        # check to see if dump returned to base
        elif self.mode == 4:

            # request to server
            self.reqeust_all_dump_team_list_flag.data = self.ex_team_number
        ...
        cmd_ang_tmp.angular.z = target_az
