...
# ros2 run multi_du_udpcomm_02_pkg du_communication_021 --ros-args --params-file src/multi_du_udpcomm_02_pkg/config/params_021.yaml
...
class MinimalPubSubComm(Node):
    ...
    def __init__(self):
        # Initialization
        super().__init__('du_communication_021')       
        
        self.declare_parameter('param_turtlePublish_communication', 'du_021/pose')
        self.declare_parameter('param_turtleSubsc_communication', 'du_021/cmd_vel')
        ...
        self.status_vessel_angle_pub = self.create_publisher(Float32, 'du_021/vessel_angle', 100)
        ...
        # Subscribers
        #self.subscription = self.create_subscription(Twist, 'du_021/cmd_vel', self.subscription_callback, 10)
        self.subscription = self.create_subscription(Twist, turtleSubsc, self.subscription_callback, 10)
        self.vessel_angular_velocity_sub = self.create_subscription(Float32, 'du_021/vessel_angular_velocity', self.vessel_angular_velocity_callback, 10)
        self.subscription_joint = self.create_subscription(JointState, 'du_021/cmd_joint', self.subscription_joint_callback, 10)
