...
# ros2 run single_ex_sim_pkg ex_communication_004
...
config.read('src/three_team_ex_stuck/config/config.ini')
...
class MinimalPubSubComm(Node):
    def __init__(self):
        # Initialization
        super().__init__('ex_communication_004')
        
        # Publishers
        self.status_joint_pub = self.create_publisher(JointState, 'ex_004/status/joint/ang', 100)
        self.ex_soilmass_pub = self.create_publisher(Float32, 'ex_004/soilmass', 100)
        ...
        # Subscribers        
        self.subscription = self.create_subscription(Twist, 'ex_004/cmd/body/vel', self.subscription_callback, 10)
        self.cmd_swing_ang_sub = self.create_subscription(Float32, 'ex_004/cmd/swing/ang', self.cmd_swing_ang_callback, 10)
        self.cmd_boom_ang_sub = self.create_subscription(Float32, 'ex_004/cmd/boom/ang', self.cmd_boom_ang_callback, 10)
        self.cmd_arm_ang_sub = self.create_subscription(Float32, 'ex_004/cmd/arm/ang', self.cmd_arm_ang_callback, 10)
        self.cmd_bucket_ang_sub = self.create_subscription(Float32, 'ex_004/cmd/bucket/ang', self.cmd_bucket_ang_callback, 10)
