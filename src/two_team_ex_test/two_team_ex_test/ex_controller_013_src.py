...
class MinimalPubSubCont(Node):
    ...
    def __init__(self):   
        # Initialization
        super().__init__('ex_controller_003') #node name
               
        # Publishers
        self.cmd_vel_pub = self.create_publisher(Twist, 'ex_003/cmd/body/vel', 100)
        self.cmd_swing_ang_pub = self.create_publisher(Float32, 'ex_003/cmd/swing/ang', 100)
        self.cmd_boom_ang_pub = self.create_publisher(Float32, 'ex_003/cmd/boom/ang', 100)
        self.cmd_arm_ang_pub = self.create_publisher(Float32, 'ex_003/cmd/arm/ang', 100)
        self.cmd_bucket_ang_pub = self.create_publisher(Float32, 'ex_003/cmd/bucket/ang', 100)
        # Dump Publishers
        self.flag_work_entry_pub = self.create_publisher(Bool,'ex_003/flag/work/entry',100)
        self.flag_dump_site_entry_pub = self.create_publisher(Bool,'ex_003/flag/dump_site/entry',100)
        self.flag_work_finish_pub = self.create_publisher(Bool,'ex_003/flag/work/finish',100)
               
        # Joy Subscribers
        self.status_joint_sub = self.create_subscription(JointState, 'ex_003/status/joint/ang',  self.status_joint_callback, 10)
        self.cmd_joy_sub = self.create_subscription(Joy, 'ex_000/cmd/joy',  self.cmd_joy_callback, 10)
        self.ex_soilmass_sub = self.create_subscription(Float32, 'ex_003/soilmass', self.ex_soilmass_callback, 100)
        # Dump Subscrivers
        self.flag_work_arrival_sub = self.create_subscription(Int16,'ex_003/flag/work/arrival',self.flag_work_arrival_callback,10)
        self.flag_work_start_sub = self.create_subscription(Bool,'ex_003/flag/work/start',self.flag_work_callback,10)
        self.flag_stack_sub = self.create_subscription(JointState,'ex_003/flag/stack',self.flag_stack_callback,10)
        self.flag_dump_site_arrival_sub = self.create_subscription(Int16,'ex_003/flag/dump_site/arrival',self.flag_dump_site_arrival_callback,10)
        self.flag_dump_site_finished_sub = self.create_subscription(Bool,'ex_003/flag/dump_site/finished',self.flag_dump_site_finished_callback,100)
    ...
    def cmd_joy_callback(self, joy_tmp):
        ...
        # auto mode
            ...
            if self.request_flag:
                ...
                if time.time() - self.interval_time > 25.0: # original 22.0
