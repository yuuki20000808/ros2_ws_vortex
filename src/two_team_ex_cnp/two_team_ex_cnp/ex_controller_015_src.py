...
class MinimalPubSubCont(Node):
    ...
    def cmd_joy_callback(self, joy_tmp):
        ...
        # auto mode
            ...
            if self.arrival_flag == False:
                #print("self.arrival_flag:", self.arrival_flag)
                #crawler_vy = 3.0
                start = (self.turtlePos[0], self.turtlePos[1])
                #goal = [(-20.0, -35.0),(-10.0, -55.0),(-5.0,-35.0),(-135.0,-55.0),(-145.0,-35.0),(-150.0,-55.0)]
                goal = [(-18.66, -34.56), (-9.0, -32.0)]

                #print("self.state_pp", self.state_pp)

                print("state_pp", self.state_pp)

                if self.state_pp == 0:
                    self.waypoint = 0
                    #twist_tmp.linear.x = 10.0 * self.healty
                    crawler_vy = 5.0 * self.healty
                ...
                if self.path is not None:
                    pure_pursuit_target_angle,self.target_index = pure_pursuit_method(self.path,self.turtlePos,self.target_index,crawler_vy)
            ...
            elif self.arrival_flag == True:
            #elif self.arrival_flag:            
                #print("self.arrival_flag:", self.arrival_flag)
