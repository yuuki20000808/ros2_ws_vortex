...
config.read('src/single_ex_sim_pkg/config/config.ini')
...
class MinimalPubSubComm(Node):
    def __init__(self):
        # Initialization
        super().__init__('ex_communication_000')
        
        # Publishers
        self.pose_pub = self.create_publisher(Pose, 'ex_000/pose', 100)
        ...
        # Subscribers
        self.subscription_joint = self.create_subscription(JointState, 'ex_000/cmd_joint', self.subscription_joint_callback, 10)
        self.subscription = self.create_subscription(Twist, 'ex_000/cmd_vel', self.subscription_callback, 10)
