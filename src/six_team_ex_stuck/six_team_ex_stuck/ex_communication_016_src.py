...
# ros2 run single_ex_sim_pkg ex_communication_006
...
UDP_IP_VORTEX = config['IP']['ip_vortex_006']
UDP_IP_ROS = config['IP']['ip_ex_006']
UDP_SEND_PORT = int(config['IP']['port_vortex_006'])
UDP_RECEIVE_PORT = int(config['IP']['port_ex_006'])
...
class MinimalPubSubComm(Node):
    def __init__(self):
        # Initialization
        super().__init__('ex_communication_006')
        
        # Publishers
        self.status_joint_pub = self.create_publisher(JointState, 'ex_006/status/joint/ang', 100)
        self.ex_soilmass_pub = self.create_publisher(Float32, 'ex_006/soilmass', 100)
        ...
        # Subscribers        
        self.subscription = self.create_subscription(Twist, 'ex_006/cmd/body/vel', self.subscription_callback, 10)
        self.cmd_swing_ang_sub = self.create_subscription(Float32, 'ex_006/cmd/swing/ang', self.cmd_swing_ang_callback, 10)
        self.cmd_boom_ang_sub = self.create_subscription(Float32, 'ex_006/cmd/boom/ang', self.cmd_boom_ang_callback, 10)
        self.cmd_arm_ang_sub = self.create_subscription(Float32, 'ex_006/cmd/arm/ang', self.cmd_arm_ang_callback, 10)
        self.cmd_bucket_ang_sub = self.create_subscription(Float32, 'ex_006/cmd/bucket/ang', self.cmd_bucket_ang_callback, 10)
