...
# ros2 run single_ex_sim_pkg ex_communication_005
...
class MinimalPubSubComm(Node):
    def __init__(self):
        ...
        # Publishers
        self.status_joint_pub = self.create_publisher(JointState, 'ex_005/status/joint/ang', 100)
        self.ex_soilmass_pub = self.create_publisher(Float32, 'ex_005/soilmass', 100)
        ...
        # Subscribers        
        self.subscription = self.create_subscription(Twist, 'ex_005/cmd/body/vel', self.subscription_callback, 10)
        self.cmd_swing_ang_sub = self.create_subscription(Float32, 'ex_005/cmd/swing/ang', self.cmd_swing_ang_callback, 10)
        self.cmd_boom_ang_sub = self.create_subscription(Float32, 'ex_005/cmd/boom/ang', self.cmd_boom_ang_callback, 10)
        self.cmd_arm_ang_sub = self.create_subscription(Float32, 'ex_005/cmd/arm/ang', self.cmd_arm_ang_callback, 10)
        self.cmd_bucket_ang_sub = self.create_subscription(Float32, 'ex_005/cmd/bucket/ang', self.cmd_bucket_ang_callback, 10)
