...
# ros2 run multi_du_udpcomm_02_pkg du_communication_010 --ros-args --params-file src/multi_du_udpcomm_02_pkg/config/params_010.yaml
...
config.read('src/four_team_du_cnp/config/config.ini')

UDP_IP_VORTEX = config['IP']['ip_vortex_021']
UDP_IP_ROS = config['IP']['ip_du_021']
UDP_SEND_PORT = int(config['IP']['port_vortex_021'])
UDP_RECEIVE_PORT = int(config['IP']['port_du_021'])
...
class MinimalPubSubComm(Node):
    ...
    def __init__(self):
        # Initialization
        super().__init__(f'du_communication_{self.ID}')       
        
        self.declare_parameter('param_turtlePublish_communication', f'du_{self.ID}/pose')
        self.declare_parameter('param_turtleSubsc_communication', f'du_{self.ID}/cmd_vel')
        ...
        self.status_vessel_angle_pub = self.create_publisher(Float32, f'du_{self.ID}/vessel_angle', 100)
        ...
        # Subscribers
        #self.subscription = self.create_subscription(Twist, f'du_{self.ID}/cmd_vel', self.subscription_callback, 10)
        self.subscription = self.create_subscription(Twist, turtleSubsc, self.subscription_callback, 10)
        self.vessel_angular_velocity_sub = self.create_subscription(Float32, f'du_{self.ID}/vessel_angular_velocity', self.vessel_angular_velocity_callback, 10)
        self.subscription_joint = self.create_subscription(JointState, f'du_{self.ID}/cmd_joint', self.subscription_joint_callback, 10)
