...
config.read('/home/swarm-team/ros2_katou_ws/src/single_ex_sim_pkg_master/config/config.ini')
...
class MinimalPubSubComm(Node):
    def __init__(self):
        # Initialization
        super().__init__('ex_communication_003')
        
        # Publishers
        self.status_joint_pub = self.create_publisher(JointState, 'ex_003/status/joint/ang', 100)
        self.ex_soilmass_pub = self.create_publisher(Float32, 'ex_003/soilmass', 100)
        ...
        # Subscribers        
        self.subscription = self.create_subscription(Twist, 'ex_003/cmd/body/vel', self.subscription_callback, 10)
        self.cmd_swing_ang_sub = self.create_subscription(Float32, 'ex_003/cmd/swing/ang', self.cmd_swing_ang_callback, 10)
        self.cmd_boom_ang_sub = self.create_subscription(Float32, 'ex_003/cmd/boom/ang', self.cmd_boom_ang_callback, 10)
        self.cmd_arm_ang_sub = self.create_subscription(Float32, 'ex_003/cmd/arm/ang', self.cmd_arm_ang_callback, 10)
        self.cmd_bucket_ang_sub = self.create_subscription(Float32, 'ex_003/cmd/bucket/ang', self.cmd_bucket_ang_callback, 10)
