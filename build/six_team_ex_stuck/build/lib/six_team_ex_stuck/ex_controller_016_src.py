...
# ros2 run single_ex_sim_pkg ex_controller_006
...
class MinimalPubSubCont(Node):
    ...
    def __init__(self):   
        # Initialization
        super().__init__('ex_controller_006') #node name
               
        # Publishers
        self.cmd_vel_pub = self.create_publisher(Twist, 'ex_006/cmd/body/vel', 100)
        self.cmd_swing_ang_pub = self.create_publisher(Float32, 'ex_006/cmd/swing/ang', 100)
        self.cmd_boom_ang_pub = self.create_publisher(Float32, 'ex_006/cmd/boom/ang', 100)
        self.cmd_arm_ang_pub = self.create_publisher(Float32, 'ex_006/cmd/arm/ang', 100)
        self.cmd_bucket_ang_pub = self.create_publisher(Float32, 'ex_006/cmd/bucket/ang', 100)
        # Dump Publishers
        self.flag_work_entry_pub = self.create_publisher(Bool,'ex_006/flag/work/entry',100)
        self.flag_work_finish_pub = self.create_publisher(Bool,'ex_006/flag/work/finish',100)
               
        # Joy Subscribers
        self.status_joint_sub = self.create_subscription(JointState, 'ex_006/status/joint/ang',  self.status_joint_callback, 10)
        self.cmd_joy_sub = self.create_subscription(Joy, 'ex_000/cmd/joy',  self.cmd_joy_callback, 10)
        self.ex_soilmass_sub = self.create_subscription(Float32, 'ex_006/soilmass', self.ex_soilmass_callback, 100)
        # Dump Subscrivers
        self.flag_work_arrival_sub = self.create_subscription(Int16,'ex_006/flag/work/arrival',self.flag_work_arrival_callback,10)
        self.flag_work_start_sub = self.create_subscription(Bool,'ex_006/flag/work/start',self.flag_work_callback,10)
        self.flag_stack_sub = self.create_subscription(JointState,'ex_006/flag/stack',self.flag_stack_callback,10)
