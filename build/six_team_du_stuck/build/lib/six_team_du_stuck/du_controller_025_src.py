...
# ros2 run four_team_du_stuck du_controller_025 --ros-args --params-file src/four_team_du_stuck/config/params_025.yaml
...
class MinimalPubSubCont(Node):
    ...
    def __init__(self):   
        # Initialization
        super().__init__('du_controller_025') #node name
                      
        # Publishers
        self.twist_pub = self.create_publisher(Twist, 'du_025/cmd_vel', 100)
        self.vessel_angular_velocity_pub = self.create_publisher(Float32, 'du_025/vessel_angular_velocity', 10)
        self.cmd_joint_pub = self.create_publisher(JointState, 'du_025/cmd_joint', 100)
        # Excavator Publishers 
        self.flag_work_start_pub = self.create_publisher(Bool,'ex_006/flag/work/start',100)
        self.flag_work_arrival_pub = self.create_publisher(Int16,'ex_006/flag/work/arrival',100)
        ...
        # Subscribers
        self.subscription = self.create_subscription(Pose, 'du_025/pose',  self.subsc_callback, 10)
        self.subscription_012 = self.create_subscription(Pose, 'du_012/pose',  self.subsc_callback_012, 10)
        self.vessel_angle_sub = self.create_subscription(Float32,'du_025/vessel_angle',self.vessel_angle_callback,10)
        # Excavator Subscribers
        self.flag_startup_sub = self.create_subscription(Bool,'du_025/flag/startup', self.flag_work_start_callback, 10)
        self.flag_work_finish_sub = self.create_subscription(Bool,'ex_006/flag/work/finish', self.flag_work_finish_callback, 10)
        self.flag_work_entry_sub = self.create_subscription(Bool,'ex_006/flag/work/entry',self.flag_work_entry_callback, 10)
        self.flag_stack_sub = self.create_subscription(JointState,'du_025/flag/stack',self.flag_stack_callback, 10)
    ...
    def timer_callback(self):
        ...
        goal = [(-20.0, 155.0),(-10.0, 175.0),(-5.0,155.0),(-135.0,175.0),(-145.0,155.0),(-150.0,175.0)]
