...
# ros2 run four_team_du_stuck du_controller_024 --ros-args --params-file src/four_team_du_stuck/config/params_024.yaml
...
class MinimalPubSubCont(Node):
    ...
    def __init__(self):   
        # Initialization
        super().__init__('du_controller_024') #node name
                      
        # Publishers
        self.twist_pub = self.create_publisher(Twist, 'du_024/cmd_vel', 100)
        self.vessel_angular_velocity_pub = self.create_publisher(Float32, 'du_024/vessel_angular_velocity', 10)
        self.cmd_joint_pub = self.create_publisher(JointState, 'du_024/cmd_joint', 100)
        ...
        # Subscribers
        self.subscription = self.create_subscription(Pose, 'du_024/pose',  self.subsc_callback, 10)
        self.subscription_012 = self.create_subscription(Pose, 'du_012/pose',  self.subsc_callback_012, 10)
        self.vessel_angle_sub = self.create_subscription(Float32,'du_024/vessel_angle',self.vessel_angle_callback,10)
        # Excavator Subscribers
        self.flag_startup_sub = self.create_subscription(Bool,'du_024/flag/startup', self.flag_work_start_callback, 10)
        ...
        self.flag_stack_sub = self.create_subscription(JointState,'du_024/flag/stack',self.flag_stack_callback, 10)
