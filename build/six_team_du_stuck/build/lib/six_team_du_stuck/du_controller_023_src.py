...
# ros2 run four_team_du_stuck du_controller_023 --ros-args --params-file src/four_team_du_stuck/config/params_023.yaml
...
class MinimalPubSubCont(Node):
    ...
    def __init__(self):   
        # Initialization
        super().__init__('du_controller_023') #node name
                      
        # Publishers
        self.twist_pub = self.create_publisher(Twist, 'du_023/cmd_vel', 100)
        self.vessel_angular_velocity_pub = self.create_publisher(Float32, 'du_023/vessel_angular_velocity', 10)
        self.cmd_joint_pub = self.create_publisher(JointState, 'du_023/cmd_joint', 100)
        # Excavator Publishers 
        self.flag_work_start_pub = self.create_publisher(Bool,'ex_005/flag/work/start',100)
        self.flag_work_arrival_pub = self.create_publisher(Int16,'ex_005/flag/work/arrival',100)
        ...
        # Subscribers
        self.subscription = self.create_subscription(Pose, 'du_023/pose',  self.subsc_callback, 10)
        self.subscription_012 = self.create_subscription(Pose, 'du_012/pose',  self.subsc_callback_012, 10)
        self.vessel_angle_sub = self.create_subscription(Float32,'du_023/vessel_angle',self.vessel_angle_callback,10)
        # Excavator Subscribers
        self.flag_startup_sub = self.create_subscription(Bool,'du_023/flag/startup', self.flag_work_start_callback, 10)
        self.flag_work_finish_sub = self.create_subscription(Bool,'ex_005/flag/work/finish', self.flag_work_finish_callback, 10)
        self.flag_work_entry_sub = self.create_subscription(Bool,'ex_005/flag/work/entry',self.flag_work_entry_callback, 10)
        self.flag_stack_sub = self.create_subscription(JointState,'du_023/flag/stack',self.flag_stack_callback, 10)
